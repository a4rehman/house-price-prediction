...
def service() -> ChurnService:
    ...
    assert app_state.service is not None  # noqa: S101
...
def predict(
    ...
    model: ChurnService = Depends(service),
) -> dict:
...
def predict_batch(
    ...
    return {
        "count": len(customers),
        "predictions": [model.predict(x.model_dump()) for x in customers],
    }
...
async def predict_csv(
    ...
    return {
        "count": len(df),
        "predictions": [
            model.predict(row.dropna().to_dict()) for _, row in df.iterrows()
        ],
    }
